...
const decodeMorse = (morseCode: string) => {
  ...
  const decodeWord = (phrase: string): string => {
  ...
  return decodeWord(morseCode);
};

decodeMorse(testData);
...
export function decodeMorse(morseCode: string): string {
  return morseCode
    ...
    .map((wordCode: string) =>
      wordCode
        .split(' ')
        .map((letterCode: string) => MORSE_CODE[letterCode])
        .join(''),
    )
    .join(' ');
}
